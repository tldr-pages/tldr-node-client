  ...
  return getShortIndex()
    .then((idx) => {
      ...
      if (hasPlatformLang(targets, preferredPlatform, preferredLanguage)) {
        ...
        console.log(`Command ${page} does not exist for the host platform. Displaying the page from ${targets[0].platform} platform`);
    ...
    .then((files) => {
      ...
      let reducer = (index, file) => {
        ...
        if (index[page]) {
          ...
          if (needsPush) {
            targets.push({'platform': platform, 'language': language});
            index[page].targets = targets;
          }
        } else {
          index[page] = {targets: [{'platform': platform, 'language': language}]};
        }
