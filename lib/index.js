...
function findPage(page, preferredPlatform, preferredLanguage) {
  ...
  return getShortIndex()
    .then((idx) => {
      ...
      const schema = bcp47.parse(preferredLanguage);
      preferredLanguage = schema.language;
      if (schema.region) {
        preferredLanguage += '-' + schema.region;
      }
...
function hasPlatformLang(targets, preferredPlatform, preferredLanguage) {
  for (const target of targets) {
    if (target.os === preferredPlatform && target.language === preferredLanguage) {
      return true;
    }
  }
  return false;
...
function hasLang(targets, preferredLanguage) {
  for (const target of targets) {
    if (target.language === preferredLanguage) {
      return true;
    }
  }
  return false;
