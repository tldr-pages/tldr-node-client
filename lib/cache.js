...
exports.update = function(opts) {
  ...
  if (parts.length == 1) {
    request.getTags(opts, function(err, tags) {
      ...
      request.getZip("https://github.com/" + config.repository + "/archive/" + tag.name + ".zip", opts, function(err, pth) {
        if (err) {
    ...
    request.getZip("https://github.com/" + parts[0] + "/archive/" + parts[1] + ".zip", opts, function(err, pth) {
      if (err) {
