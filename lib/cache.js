...
exports.list = function(done) {
  var files = wrench.readdirSyncRecursive(config.get().cache);
...
function tryLoad(pages, index, done) {
  ...
  var filepath = path.join(config.get().cache, pages[index]);
...
function lastUpdate() {
  try {
    return fs.statSync(config.get().cache).mtime;
  } catch (ex) {
    return 0;
  }
}
