...
const importSafe = require('./utils').importSafe;
const natural = importSafe('natural', (err, query) => {
  if (err.code !== 'ERR_REQUIRE_NATIVE') throw err;
  const rootPath = path.join(__dirname, '../');
  console.error(chalk`
    {bgRed.whiteBright Error} Executing {bold.cyan tldr} failed due to incorrect native dependencies
      target: {blue ${path.relative(rootPath, err.filename)}}
      required by: {blue ${path.relative(rootPath, require.resolve(query))}}

    Please rebuild your {bold.cyan tldr} installation using:
    $ {whiteBright cd ${rootPath}; npm rebuild}
  `);
  throw err;
});
