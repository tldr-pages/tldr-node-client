...
let getTokens = (data) => {
  ...
  let newwords = tokenizer.tokenize(data).filter(e => {return e.indexOf('-') > 0 && e.indexOf('-') < e.length - 1;});
  // console.log(newwords);
  tokens = tokens.concat(newwords);
  nwords.push(newwords);
  tokens.forEach((word, index) => {
    word = word.toLowerCase();
    // if(word.startsWith('in-')) console.log("HERE");
    word = natural.PorterStemmer.stem(word);
    tokens[index] = word;
  });
  if(tokens.indexOf('in-plac') != -1) {
